...
describe("Test GET requests to the data provider", () => {
  ...
  it("returns timeout error object when request times out", async () => {
    ...
    const request = new FetchRequest();
    const item = await request.getObject(
      "/items/test-item/",
      undefined,
      {},
      100
    ...
    expect(item["@type"]).toContain("TimeOutError");
    expect(item.status).toEqual("error");
    expect(item.code).toEqual("TIMEOUT");
    expect(FetchRequest.isResponseSuccess(item)).toBeFalsy();
  });
});
...
describe("Test URL-specific fetch requests", () => {
  ...
  it("returns timeout error object when request times out", async () => {
    ...
    const request = new FetchRequest();
    const item = await request.getObjectByUrl(
      "/items/test-item/",
      undefined,
      {},
      100
...
describe("Text fetch requests", () => {
  ...
  it("returns timeout error object when request times out", async () => {
    ...
    const request = new FetchRequest();
    const item = await request.getText("/items/test-item/", undefined, {}, 100);
    expect(item).toBeTruthy();
    expect(item["@type"]).toContain("TimeOutError");
    expect(item.status).toEqual("error");
    expect(item.code).toEqual("TIMEOUT");
...
describe("Test getTextByUrl requests", () => {
  it("returns text with a successful request", async () => {
    const mockData = "## Markdown";
    window.fetch = jest.fn().mockImplementation(() =>
      Promise.resolve({
    ...
    const request = new FetchRequest();
    const markdown = await request.getTextByUrl("http://encode.org/path");
    expect(markdown).toEqual(mockData);
  });
  ...
  it("returns a default value when text request unsuccessful", async () => {
    ...
    const request = new FetchRequest();
    const markdown = await request.getTextByUrl(
      "http://encode.org/path",
      "Error Message"
    );
    expect(typeof markdown).toEqual("string");
    expect(markdown).toEqual("Error Message");
  ...
  it("returns timeout error object when request times out", async () => {
    ...
    const request = new FetchRequest();
    const item = await request.getTextByUrl(
      "/items/test-item/",
      undefined,
      {},
      100
