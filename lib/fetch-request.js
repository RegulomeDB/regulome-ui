...
// lib
import { API_URL_GDS } from "./constants";

// Default 20-second timeout for fetch requests
const TIMEOUT = 20000;
...
/**
 * Standard returned response for a request time out error.
 */
const REQUEST_TIME_OUT_ERROR_RESPONSE = {
  "@type": ["TimeOutError", "Error"],
  status: "error",
  code: "TIMEOUT",
  title: "request time out error",
  description: "The request is taking too long.",
  detail: "The request is taking too long.",
};
Object.freeze(REQUEST_TIME_OUT_ERROR_RESPONSE);
...
export default class FetchRequest {
   ...
   * @param {number} timeout Time in milliseconds before request times out
   * @returns {object} Requested object or error object
   */
  async getObject(path, defaultErrorValue, options = {}, timeout = TIMEOUT) {
    const controller = new AbortController();
    const { signal } = controller;
    const headerOptionsNoSignal = this.#buildOptions(FETCH_METHOD.GET, {
      accept: PAYLOAD_FORMAT.JSON,
    });
    const headerOptions = { ...headerOptionsNoSignal, signal };
    ...
    try {
      ...
      if (error.name === "AbortError") {
        return REQUEST_TIME_OUT_ERROR_RESPONSE;
      }
   ...
   * @returns {object} Requested object or error object
   */
  async getObjectByUrl(url, defaultErrorValue, timeout = TIMEOUT) {
    const controller = new AbortController();
    const { signal } = controller;
    const headerOptionsNoSignal = this.#buildOptions(FETCH_METHOD.GET, {
      accept: PAYLOAD_FORMAT.JSON,
   ...
   */
  async getText(path, defaultErrorValue, timeout = TIMEOUT) {
    const controller = new AbortController();
    const { signal } = controller;
    const optionsNoSignal = this.#buildOptions(FETCH_METHOD.GET, {
      accept: PAYLOAD_FORMAT.TEXT,
    });
    const options = { ...optionsNoSignal, signal };
    const timeoutId = setTimeout(() => controller.abort(), timeout);
    try {
  ...
  /**
   * Request text file string with the given url.
   * @param {string} url url to the requested resource
   * @param {*} defaultErrorValue Value to return if the request fails; error object if not given
   * @param {number} timeout Time in milliseconds before request times out
   * @returns {string/object} Requested string, or error object if `defaultErrorValue` not given
   */
  async getTextByUrl(url, defaultErrorValue, timeout = TIMEOUT) {
    const controller = new AbortController();
    const { signal } = controller;
    const optionsNoSignal = this.#buildOptions(FETCH_METHOD.GET, {
      accept: PAYLOAD_FORMAT.TEXT,
