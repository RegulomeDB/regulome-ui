...
/**
 * This function get filtered data
 ...
 */
export function getOrganFacets(data, normalizedTissueSpecificScore) {
  const organForData = [];
  data.forEach((dataset) => {
    const organSlim = dataset.biosample_ontology?.organ_slims
      ? dataset.biosample_ontology.organ_slims
      : [];
    organSlim.forEach((organ) => {
      if (!(organ in organForData)) {
        organForData.push(organ);
      }
    });
  });
  const organFacet = {};
  organForData.forEach((organ) => {
    if (organ in normalizedTissueSpecificScore) {
      organFacet[organ] = normalizedTissueSpecificScore[organ];
    }
  });
  return organFacet;
}
