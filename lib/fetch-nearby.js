 ...
 * The data we need are: the coordinates of the display region, the nearest genes
 * and the regulatory regions in the display region.
 ...
 * @param {object} request to fetch data
 * @param {string} region for query
 * @returns a list of regulatory regions for the given region
 */
async function getRegulatoryRegions(request, region) {
  ...
  let total = [];
  do {
    const query = `${API_URL_REGULATORY_REGION}organism=human&region=${region}&page=${pageNum}&limit=1000`;
