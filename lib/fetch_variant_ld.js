...
export default async function fetchVariantLD(
  ...
  if (assembly === "GRCh38") {
    const variants = await getVariantsByRegion(request, region);
    ...
    let variantLD = await Promise.all(
      variants.map(async (variant) => {
        ...
        const dataFormatted = data.map((pair) => {
          const sequenceVariant = pair.sequence_variant[0];
          const start = parseInt(sequenceVariant.pos);
          const end = start + 1;
          return {
            chr: sequenceVariant.chr,
            start,
            end,
            ancestry: pair.ancestry,
            r2: pair.r2,
            query_spdi: variant.spdi,
            rsid: sequenceVariant.rsid,
            location: `${sequenceVariant.chr}:${start}-${end}`,
            ref: sequenceVariant.ref,
            alt: sequenceVariant.alt,
            assembly,
          };
        });
 ...
 * The max number of results we can get per page from catalog is 500
...
async function getVariantsByRegion(request, region) {
  ...
  let total = [];
  do {
    const query = `${API_URL_VARIANT}region=${region}&page=${pageNum}&limit=500`;
...
/**
 * This function get all the SNPs type of variants in LD for given variant id
 * The max number of results we can get per page from catalog is 500
 * @param {object} request to fetch data
 * @param {string} region for query
 * @returns all the variants in LD for given variant id in catalog database
 */
async function getVariantsInVariantLD(request, id, r2, ancestry) {
  ...
  do {
    const query = `${API_URL_VARIANT_LD}variant_id=${id}&verbose=true&organism=Homo%20sapiens&r2=gte:${
      r2 >= R2 ? r2 : R2
    }&page=${pageNum}${ancestryParam}&limit=${maxLimit}`;
    variants = await request.getObjectByUrl(query);
    if (ERRORS.includes(variants.code)) {
      return variants;
    }
    total = total.concat(variants);
    pageNum += 1;
  } while (variants.length === maxLimit);
  // filter out the variants that are not SNPs
  total = total.filter(
    (pair) =>
      pair.sequence_variant[0].ref.length === 1 &&
      pair.sequence_variant[0].alt.length === 1
  );
  total.sort(
    (a, b) =>
      parseInt(a.sequence_variant[0].pos) - parseInt(b.sequence_variant[0].pos)
  );
  return total;
}
...
async function getVariantsScore(request, variants, assembly) {
  ...
  do {
    const regionsParm = regions
      ...
      .join(" ");
    const query = `${API_URL_GDS_SCORE}regions=${regionsParm}&genome=${assembly}`;
    const scores = await request.getObjectByUrl(query);
    // if the query type is search, there is only one variant in variants.
    // And the regulome score is in the top level, we need to move it to the variants
    if (scores["@type"][0] === "search") {
      scores.variants[0].regulome_score = scores.regulome_score;
    }
