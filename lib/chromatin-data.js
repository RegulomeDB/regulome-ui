...
/**
 * Return fill color for chromatin state body map
 */
export function getFillColorChromatin(facets, organ) {
  const fill =
    organ in facets ? ChromatinStateColor[facets[organ][0]].fill : "fill-white";

  return fill;
}

/**
 * Return background color in tailwind style
 */
export function getBackgroundColorChromatin(facets, organ) {
  const fill =
    organ in facets ? ChromatinStateColor[facets[organ][0]].background : null;
  return fill;
}
